/*
A mutex is created so that multiple threads don't access the variable at the same time and create a race condition.
The threads lock and unlock the mutex when executing a critical section.
The program prints messages from threads that pass in an even number first and those that pass in an odd number after all even have been printed.
*/
